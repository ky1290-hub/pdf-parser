...
> **Status:** v1.3.0 public-plugin submission package under the MIT License.
>
...
Download `pdf-parser-plugin-v1.3.0.zip` from the latest GitHub release. The same archive includes the portable root `plugin.json`, `.codex-plugin/plugin.json` for Codex compatibility, `.claude-plugin/plugin.json` for Claude Code, and the shared skill under `skills/pdf-parser/`.
...
1. Download `pdf-parser-claude-v1.3.0.zip` from the latest GitHub release.
...
Download `pdf-parser-codex-v1.3.0.zip`, extract it, and place the `pdf-parser` folder under `$CODEX_HOME/skills/` or `~/.codex/skills/`. The Codex package includes `agents/openai.yaml`.
